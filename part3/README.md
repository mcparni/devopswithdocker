...
Diagram shows a cluster which has a load balancer. Enduser can send requests only to load balancer. Load balancer serves front end according to load. Front end has volume mounted for files. front ends can talk with backend. Backends talk with front end and can also access to database. Backends have mounted the volume too for persistend database and other files.
